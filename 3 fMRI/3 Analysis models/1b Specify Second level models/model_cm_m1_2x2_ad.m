function [ matlabbatch contrastfiles secondlevelfolder] = model_cm_m1_2x2_ad(where_brain, secondlevelfolder, subjectlist, analysistype, firstlevelmodel, covariates, memtype,memdata)
% [ matlabbatch contrastfiles secondlevelfolder] = model_cm_m1_2x2_ad(where_brain, secondlevelfolder, subjectlist, analysistype, firstlevelmodel, covariates, memtype,memdata)
% Second level model, for basic 2x2 Factorial (Sim x Val)
%
% Options for memtype:      'Hit', 'Surehit', 'Rem', 'Surerem'
%
% This second level model includes 1st level contrasts that weight all 3
% derivatives for each condition. '_ad' = all derivatives
% -------------------------------------------------------------------------------
...
for s=1:length(subjectlist) 
    ...
    for i=1:length(factorialcells)
        ...
        if strcmp(memtype,'Roc')==1
            [ws.contrastnames] = f_findcontrastname(ws.SPM, {[factorialcells{i,1} 'xCMem_' memtype '_ad'] });
        else
            [ws.contrastnames] = f_findcontrastname(ws.SPM, {[factorialcells{i,1} 'xCMem_' memtype{1} '_ad'] });
        end
