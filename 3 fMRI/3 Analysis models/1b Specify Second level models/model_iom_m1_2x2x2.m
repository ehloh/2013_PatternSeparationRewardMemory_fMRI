function [ matlabbatch contrastfiles secondlevelfolder] = model_iom_m1_2x2x2(where_brain, secondlevelfolder, subjectlist, analysistype, firstlevelmodel, covariates, memtype,memdata)
...
% where_brain=where.data_brain; subjectlist=log.subjects; firstlevelmodel=log.firstlevelmodel; secondlevelmodel=log.secondlevelmodel;  memtype=w.memtype;
...
factorialcells={'SR_m1' [1 1 1 ];           % Contrasts names must match the names specified in the 1st level
                        'SR_m0' [1 1 2]; 
                        'SN_m1' [1 2 1];  
                        'SN_m0' [1 2 2]; 
                        'DR_m1' [2 1 1]; 
...
matlabbatch{1}.spm.stats.factorial_design.des.fd.fact(3).name = ['mem' memtype{1}];
