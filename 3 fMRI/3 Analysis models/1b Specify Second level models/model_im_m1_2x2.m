function [ matlabbatch contrastfiles secondlevelfolder] = model_im_m1_2x2(where_brain, secondlevelfolder, subjectlist, analysistype, firstlevelmodel, covariates, memtype,memdata)
% [ matlabbatch contrastfiles secondlevelfolder] = model_im_m1_2x2(where_brain, secondlevelfolder, subjectlist, analysistype, firstlevelmodel, covariates, memtype,memdata)
...
%% (1) Details for this model: Factorial design (Sim x Val)
...
factorialcells={'SR_ItemxMem' [1 1];
                        'SN_ItemxMem' [1 2];
                        'DR_ItemxMem' [2 1];
                        'DN_ItemxMem' [2 2];};
...
for s=1:length(subjectlist) 
    ...
    for i=1:size(factorialcells,1)
        ...
        if strcmp(memtype,'Roc')==1
            [ws.contrastnames] = f_findcontrastname(ws.SPM, {   [factorialcells{i,1} '_' memtype] });
        else
            [ws.contrastnames] = f_findcontrastname(ws.SPM, {   [factorialcells{i,1} '_' memtype{1}] });
        end
